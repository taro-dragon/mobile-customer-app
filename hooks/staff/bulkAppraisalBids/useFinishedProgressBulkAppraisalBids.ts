...
const fetchFinishedBulkAppraisalBids = async (
  storeId: string,
  lastDoc?: any
...
const useFetchFinishedBulkAppraisalBids = () => {
  ...
  const { data, error, size, setSize, isValidating, isLoading, mutate } =
    useSWRInfinite(
      (pageIndex: number, previousPageData: any) => {
        ...
        const key =
          pageIndex === 0
            ? [`finishedBulkAppraisalBids-${storeId}`, storeId, null]
            : !previousPageData?.hasMore
            ? null
            : [
                `finishedBulkAppraisalBids-${storeId}`,
                storeId,
      ...
      ([_, storeId, lastDoc]: [string, string, any]) =>
        fetchFinishedBulkAppraisalBids(storeId, lastDoc),
      {
...
export default useFetchFinishedBulkAppraisalBids;
