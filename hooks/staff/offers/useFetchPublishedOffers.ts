  ...
  const { data, error, size, setSize, isValidating, isLoading, mutate } =
    useSWRInfinite(
      (pageIndex: number, previousPageData: any) => {
        ...
        const key =
          pageIndex === 0
            ? [`publishedOffers-${storeId}`, storeId, null]
            : !previousPageData?.hasMore
            ? null
            : [`publishedOffers-${storeId}`, storeId, previousPageData.lastDoc];
        return key;
      },
      ([_, storeId, lastDoc]: [string, string, any]) =>
        fetchPublishedOffers(storeId, lastDoc),
      {
...
export default usePublishedOffers;
