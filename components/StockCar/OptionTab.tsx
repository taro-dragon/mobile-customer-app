...
const OptionTab: React.FC<OptionTabProps> = ({ stockCar }) => {
  const { colors, typography } = useTheme();

  const renderOptionItem = (optionName: string, label: string) => {
    const isEnabled = stockCar[optionName as keyof StockCar] === true;
    return (
      <View
        key={optionName}
        style={[
  ...
  const renderModalPickerItem = (pickerName: string, label: string) => {
    const value = stockCar[pickerName as keyof StockCar] as string;
    const hasValue =
